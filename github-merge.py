...
def print_merge_details(pull_reference, title, branch, base_branch, head_branch, acks, message):
    ...
    subprocess.check_call([GIT, '--no-pager', 'log', '--graph', '--topo-order',
                           '--no-show-signature', '--no-decorate',
                           '--no-abbrev-commit', '--pretty=tformat:' + COMMIT_FORMAT,
...
def main():
    ...
    try:
        subprocess.check_call([GIT, '--no-pager', 'log', '-q', '-1',
                               '--no-show-signature', '--no-decorate',
                               '--no-abbrev-commit', 'refs/heads/' + head_branch],
                              stdout=devnull, stderr=stdout)
        head_commit = subprocess.check_output([GIT, '--no-pager', 'log', '-1',
                                               '--pretty=format:%H',
                                               '--no-show-signature',
                                               '--no-decorate',
                                               '--no-abbrev-commit',
                                               head_branch]).decode('utf-8')
    ...
    try:
        subprocess.check_call([GIT, '--no-pager', 'log', '-q', '-1',
                               '--no-show-signature', '--no-decorate',
                                '--no-abbrev-commit', 'refs/heads/' + merge_branch],
                              stdout=devnull, stderr=stdout)
    except subprocess.CalledProcessError:
    ...
    try:
        ...
        message = firstline + '\n\n'
        message += subprocess.check_output([GIT, '--no-pager', 'log',
                                            '--no-merges', '--topo-order',
                                            '--no-show-signature',
                                            '--no-decorate',
                                            '--no-abbrev-commit',
        ...
        try:
            ...
            sys.exit(4)
        logmsg = subprocess.check_output([GIT, '--no-pager', 'log',
                                          '--pretty=format:%s', '-n', '1',
                                          '--no-show-signature',
                                          '--no-decorate',
                                          '--no-abbrev-commit',]).decode('utf-8')
